...
@on(
    ...
    tests={
        ...
        ): Warn(pattern="attach param"),
        Input(command="mv crash-4821.panic docs/reports/crash-4821.panic"): Warn(pattern="log_add tool"),
...
@on(
    ...
    max_fires=NUDGE_MAX_FIRES,
    tests={
        Input(command='cc-notes doc add "Handoff — full detail in session scratchpad steering-handoff.md" --when w'): Warn(pattern="attach param"),
...
@on(
    ...
    max_fires=NUDGE_MAX_FIRES,
    tests={
        Input(tool="ExitPlanMode"): Warn(pattern="task_add tool"),
        Input(tool="Edit", file="m.py"): Allow(),
    },
)
