...
func TestHelperReleasePinsExactVersionURLAndDigest(t *testing.T) {
	setHelperRelease(t, "v0.41.0", "0.41.0", strings.Repeat("ab", 32))
	release, err := helperRelease()
	if err != nil {
		t.Fatal(err)
	}
	if release.Version != "0.41.0" {
		t.Fatalf("version = %q", release.Version)
	}
	if release.URL != "https://github.com/yasyf/cc-notes/releases/download/v0.41.0/cc-notes-helper-v0.41.0-darwin.zip" {
...
func TestHelperReleaseMatchesPrereleaseBundleMarketingVersion(t *testing.T) {
	setHelperRelease(t, "v0.41.0-rc.1", "0.41.0", strings.Repeat("ab", 32))
	release, err := helperRelease()
	if err != nil {
		t.Fatal(err)
	}
	...
	if release.URL != "https://github.com/yasyf/cc-notes/releases/download/v0.41.0-rc.1/cc-notes-helper-v0.41.0-rc.1-darwin.zip" {
		t.Fatalf("asset URL = %q", release.URL)
	}
}

func TestHelperReleaseRejectsMissingDigest(t *testing.T) {
	setHelperRelease(t, "v0.41.0", "0.41.0", "")
...
func TestHelperReleaseRejectsInvalidBundleVersion(t *testing.T) {
	for _, value := range []string{"", " 0.41.0"} {
		t.Run(value, func(t *testing.T) {
			setHelperRelease(t, "v0.41.0", value, strings.Repeat("ab", 32))
			if _, err := helperRelease(); err == nil {
...
func TestReconcileHelperCallsFetcherWhenExecutableAlreadyExists(t *testing.T) {
	setHelperRelease(t, "v0.41.0", "0.41.0", strings.Repeat("cd", 32))
...
func TestReconcileHelperReturnsFetchFailure(t *testing.T) {
	setHelperRelease(t, "v0.41.0", "0.41.0", strings.Repeat("ef", 32))
	t.Setenv("HOME", t.TempDir())
