...
func TestReleasePackagesHelperWithoutPublishingRuntimeCask(t *testing.T) {
	...
	extract := strings.Index(string(formulaPayload), `system "/usr/bin/ditto", "-x", "-k"`)
	verify := strings.Index(string(formulaPayload), `system "/usr/bin/codesign", "--verify"`)
...
func TestPluginBootstrapEnforcesV054WithoutInstallingService(t *testing.T) {
	root := filepath.Join("..", "..")
	...
	if _, err := os.Stat(marker); err != nil {
		t.Fatalf("v0.53.1 did not trigger binary upgrade: %v", err)
	}
	...
	if _, err := os.Stat(marker); !os.IsNotExist(err) {
		t.Fatalf("v0.54.0 unexpectedly triggered binary upgrade: %v", err)
	}
}
