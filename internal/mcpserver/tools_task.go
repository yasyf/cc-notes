...
func registerTask(ts *toolset, b *bridge) {
	...
	idTool(ts, b, "task_show", "Show one task with its full detail and derived blocks index.", "task", "show")

	addTool(ts, &mcp.Tool{Name: "task_start", Description: "Claim a task and set its branch to the current HEAD branch or an explicit branch."},
	...
	addTool(ts, &mcp.Tool{Name: "task_claim", Description: "Claim a task (lease it). Use steal for an expired lease, sync to yield if another agent won."},
	...
	addTool(ts, &mcp.Tool{Name: "task_validate", Description: "Run a task's stored validation scripts. Requires yes to execute untrusted script content."},
